...
@section('content')
<h1 class="fw-200">{{ $province->name }}</h1>
<form method="post" action="{{ route('admin.provinces.update', $province->id) }}">
    ...
    <div class="form-group">
        ...
        <input type="number" class="form-control @error('weight') is-invalid @enderror" id="weight" name="weight" value="{{ old('weight', $province->weight) }}" required>
        @error('weight')
        <span class="invalid-feedback" role="alert">
            <strong>{{ $message }}</strong>
        </span>
